...
- Memory is (on all common systems) byte addresses, meaning that each byte in
  memory has its own address.  Most types are larger than 1 byte though -- you
  may remember that `int`s are 4 bytes (on current 64-bit Intel machines, like
  we're using) -- so memory won't typically have one value stored per address,
  as in the picture: it will instead have 1 `int` taking up 4 slots.  We often
  draw it with one `int` per address though, unless we're worried about what
  the actual bytes look like, because it's easier that way.
...
- Arrays are *zero indexed*: that is, the first element of the array `int
  ...
  the number inside the braces the "index" or the "offset".  It's helpful to
...
- A *pointer* is a variable that contains the memory address of a variable.
  Recall (it's important!) that variables are stored in memory, and each byte
  in memory has its own address.
...
int main() {
    ...
    cout << endl;
    cout << "first element of a: " << *a << endl;
    cout << "first element of a: " << *(&a[0]) << endl;
    cout << "first element of a: " << a[0] << endl;
    cout << endl;
    cout << "third element of a: " << *(a+2) << endl;
    cout << "third element of a: " << *(&a[0]+2) << endl;
    cout << "third element of a: " << a[2] << endl;
...
array elements that don't exist.**  Thank you :)
...
```C++
    string words[5] = {"zero", "one", "two", "three", "four"};
    cout << words[num%5];
```
...
do these sorts of things, usually using arrays, and with how long these types
of things tend to take.
...
print out all the words that are palindromes.  Or at least (since that would
likely take a long time) maybe think about how that might be accomplished :) .
